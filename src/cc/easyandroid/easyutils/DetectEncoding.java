...
public class DetectEncoding {
	...
	private final int GBK = 1;
	private final int HZ = 2;
	private final int BIG5 = 3;
	private final int EUC_TW = 4;
	private final int ISO_2022_CN = 5;
	private final int UTF8 = 6;
	private final int UNICODE = 7;
	private final int ASCII = 8;
	private final int OTHER = 9;

	private final int TOTAL_ENCODINGS = 10;
	...
	public String detectEncoding(File testfile) {
		...
		// KplayerLog.i("detect_encoding", "rawText is " + rawtext);
		try {
		...
		int encode = detectEncoding(rawtext);
		return getEncode(encode);
	}
	...
	private int detectEncoding(byte[] rawtext) {
		...
		// KplayerLog.i("detect_encoding"," maxscore is " + maxscore);
		// Return OTHER if nothing scored above 50
		if (maxscore <= 50) {
			encoding_guess = OTHER;
		}
		// KplayerLog.d("detect_encoding", "encoding_guess is " +
		// encoding_guess);
		return encoding_guess;
	}

	private String getEncode(int encode) {
		return nicename[encode];
	}
	...
	int gb2312_probability(byte[] rawtext) {
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			// KplayerLog.d("detect_encoding","rawtext["+i+"] is " +
			// rawtext[i]);
			if (rawtext[i] >= 0) {
				// asciichars++;
			} else {
		...
		// KplayerLog.i("detect_encoding", "gbchars is " + gbchars
		// +"  dbchars is " + dbchars
		// +"  gbfreq is " + gbfreq
		// +"  totalfreq is " + totalfreq);
		rangeval = 50 * ((float) gbchars / (float) dbchars);
		freqval = 50 * ((float) gbfreq / (float) totalfreq);
		// KplayerLog.d("detect_encoding", "rangeval + freqval is " + rangeval +
		// freqval);
